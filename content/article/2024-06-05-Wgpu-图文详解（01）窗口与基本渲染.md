...
为了不赘述项目搭建的过程细节，读者可以直接使用本系列的项目仓库下[ch00_simple_winit目录](https://github.com/w4ngzhen/wgpu_winit_example/tree/main/ch00_simple_winit)中的内容进行初始搭建。下面是一个基本的项目结构，以及其中的内容： ![010-init-project-arch](https://res.zhen.wang/images/post/2024-06-05/010-init-project-arch.png)

如果初始项目没有问题，我们的运行以后应该有一个独立空白的窗口显示了出来：

![020-a-simple-window](https://res.zhen.wang/images/post/2024-06-05/020-a-simple-window.png)
...
![030-create-wgpu-ctx-mod](https://res.zhen.wang/images/post/2024-06-05/030-create-wgpu-ctx-mod.png)

基于上述考虑，我们创建一个名为`wgpu_ctx.rs`的文件，并且在该文件中创建一个名为`WgpuCtx`的结构体，并将相关的Wgpu的构建代码放到这个文件中。此时代码如下：

![040-wgpu-ctx-file](https://res.zhen.wang/images/post/2024-06-05/040-wgpu-ctx-file.png)
...
![050-create-surface](https://res.zhen.wang/images/post/2024-06-05/050-create-surface.png)
...
![060-get-adapter-and-device](https://res.zhen.wang/images/post/2024-06-05/060-get-adapter-and-device.png)
...
![070-basic-webgpu-stack](https://res.zhen.wang/images/post/2024-06-05/070-basic-webgpu-stack.png)
...
![080-get_default_config](https://res.zhen.wang/images/post/2024-06-05/080-get_default_config.png)
...
![090-config-surface](https://res.zhen.wang/images/post/2024-06-05/090-config-surface.png)
...
![100-switch-surface-size](https://res.zhen.wang/images/post/2024-06-05/100-switch-surface-size.gif)
...
![110-6-components](https://res.zhen.wang/images/post/2024-06-05/110-6-components.png)
...
![120-struct-lifetime](https://res.zhen.wang/images/post/2024-06-05/120-struct-lifetime.png)
...
![130-new-func-modify](https://res.zhen.wang/images/post/2024-06-05/130-new-func-modify.png)
...
![140-wgpu-ctx-rs](https://res.zhen.wang/images/post/2024-06-05/140-wgpu-ctx-rs.png)

## 关于窗体的引用问题

目前为止，我们完成了Wgpu上下文的代码调整，为了满足后续代码流程，我们需要在原先的`app.rs`中的`App`结构体中存储创建出来的上下文数据，这块的内容调整如下：

![150-modify-app-rs](https://res.zhen.wang/images/post/2024-06-05/150-modify-app-rs.png)

此时，当我们进行编译的时候会发现有如下报错：

![160-compile-err](https://res.zhen.wang/images/post/2024-06-05/160-compile-err.png)
...
![170-dangling-ref-err](https://res.zhen.wang/images/post/2024-06-05/170-dangling-ref-err.png)
...
![180-window-move-to-ctx](https://res.zhen.wang/images/post/2024-06-05/180-window-move-to-ctx.png)
...
![190-arc-window](https://res.zhen.wang/images/post/2024-06-05/190-arc-window.png)
...
![200-draw-flow](https://res.zhen.wang/images/post/2024-06-05/200-draw-flow.png)
...
![210-swap-chain](https://res.zhen.wang/images/post/2024-06-05/210-swap-chain.png)
...
![220-command-encoder-flow](https://res.zhen.wang/images/post/2024-06-05/220-command-encoder-flow.png)
...
![230-wgpu-ctx-arch](https://res.zhen.wang/images/post/2024-06-05/230-wgpu-ctx-arch.png)

接下来，我们需要在实现了`ApplicationHandler`trait的`app.rs`中增加调用`WgpuCtx`的`draw`的代码，毕竟只有调用了该`draw`方法，内容才会真正呈现出来，具体代码如下：

![240-call-draw-simple](https://res.zhen.wang/images/post/2024-06-05/240-call-draw-simple.png)
...
![250-resize-wrong-draw](https://res.zhen.wang/images/post/2024-06-05/250-resize-wrong-draw.gif)

其实，聪明的读者已经想到了，我们每次渲染内容，都是使用的由窗口初始大小尺寸构造的表面配置：

![260-size-config-never-change](https://res.zhen.wang/images/post/2024-06-05/260-size-config-never-change.png)

为了将窗口的尺寸变化同步到表面配置中，我们给`WgpuCtx`新增一个API`resize`，同时，在原先`Resized`事件处理代码块中，在进行`request_redraw`前，先进行一次表面配置中尺寸的同步操作：

![270-keep-size-sync](https://res.zhen.wang/images/post/2024-06-05/270-keep-size-sync.png)

至此，我们基本完成了本文的所有代码，再次运行本应用，能够看到最终的效果：

![280-final](https://res.zhen.wang/images/post/2024-06-05/280-final.gif)
