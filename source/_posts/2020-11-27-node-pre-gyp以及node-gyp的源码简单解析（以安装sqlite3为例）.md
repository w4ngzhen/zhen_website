...
![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/hosted_tarball_download.png)

根据流程，接下来我们进一步检查`versioning.js`文件，找到其中的`evaluate`函数，分析最后的`hosted_tarball`路径：

![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/analysis_hosted_path.png)
...
经过源码分析来源路径为：
 ...
 ![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/opts.hosted_path-analysis.png)
...
![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/import-gyp.png)
...
![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/sqlite3-binary.png)
...
![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/opts.package_name-analysis.png)
...
![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/gyp-from-install.png)

那么我们又将回到调用install的地方。实际上，gyp就是node-pre-gyp.js导出的模块：

![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/node-pre-gyp-export.png)

也就是说在`do_build`中进行操作就是，放置了一个`build`任务在队列中。所以我们按照先前的分析，直接去看`build.js`

![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/build-buildjs.png)

看源码调用了当前模块中的`do_build`，且其中最核心的就是`compile`模块：

![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/do_build-buildjs.png)

#### util/compile.js

进入compile模块，直接找到对应的`run_gyp`函数，代码很短，不难看出进行构建调用了`node-gyp`

![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/run-gyp.png)
...
![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/node-gyp-GYP.png)
...
![](https://res.zhen.blog/images/post/2020-11-27-node-native-install/node-gyp-build-flow.png)
